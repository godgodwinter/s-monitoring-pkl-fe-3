...
const doStoreData = async (d) => {
    ...
    try {
        ...
        router.push({ name: "admin-pendaftaran-list-pemberkasan" });
        // resetForm();
...
const getPilihanSiswa = async () => {
    try {
        const response = await Api.get(
            `kaprodi/pendaftaranpkl/getdatasiswa?cari=${inputCariSiswa.value
            }&tersedia=${inputStatusSiswa.value ? inputStatusSiswa.value.label : ""
            }&tempatpkl_id=${tempatPklTerpilih.value.id}`
        );
        // data.value = response.data;
...
<template>
    ...
    <div class="bg-white shadow-sm rounded-sm py-4 px-4 border">
        <div class="px-2 py-2 space-x-2">
            <button @click="btnTempatPKLClick()" :class="container == 'tempatPKL' ? 'bg-sky-200' : ''"
            ...
            <button @click="btnSiswaClick()" :class="container == 'siswa' ? 'bg-sky-200' : ''"
                ...
                <div class="card w-96 bg-base-100 shadow-xl" v-if="tempatPklTerpilih">
                    ...
                    <div class="card-body">
                        ...
                        <div class="card-actions justify-end">
                            <div class="badge badge-outline" v-if="tempatPklTerpilih.tersedia > 0">
                                Tersedia
                            </div>
                            <div class="badge badge-outline" v-else="tempatPklTerpilih.tersedia < 1">
                                Tidak Tersedia
                            </div>
                        ...
                        <div class="card-actions justify-end">
                            <button class="btn btn-warning btn-warning-content" @click="removeTempatPkl()">
                                Tutup
                            </button>
                        </div>
                    </div>
    ...
    <div class="bg-white shadow-sm rounded-sm py-4 px-4 border pt-10 mt-2">
        <div id="containerTempatPrakerin" v-if="container == 'tempatPKL'">
            ...
            <div id="body">
                ...
                <div id="bodyContent" class="text-gray-700">
                    <div class="grid sm:grid-cols-1 md:grid-cols-2 2xl:grid-cols-2 gap-2 text-sm">
                        <!-- <img src="@/assets/img/photo/company-1.jpg" alt="company-1.jpg" /> -->
                        <div class="" v-for="(item, index) in tempatPKL">
                            <div class="card card-side bg-base-100 shadow-xl h-72" :key="item.id">
                                <figure>
                                    <img src="@/assets/img/photo/company-1.jpg" class="object-cover h-full w-32"
                                        alt="Tempat PKL" />
                                </figure>
                                <div class="card-body">
                                    ...
                                    <div class="card-actions justify-end">
                                        <div class="badge badge-outline tooltip" data-tip="Status Tempat PKL">
                                            {{ item.status }}
                                        </div>
                                        <div class="badge badge-outline tooltip" data-tip="Terisi / Kuota">
                                            {{ item.terisi }}/{{ item.kuota }}
                                        </div>
                                    </div>
                                    <div class="card-actions justify-end">
                                        <button class="btn btn-primary btn-primary-content" @click="
                                            setTempatPkl(
                                                index,
                                                item.id,
                                                item.nama,
        ...
        <div id="containerTempatPrakerin" v-if="container == 'siswa'">
            ...
            <div id="body">
                <div id="bodyHeader" class="flex flex-col md:flex-row justify-center gap-2">
                    <v-select class="py-2 px-3 w-72 mx-auto md:mx-0" :options="statusSiswa" v-model="inputStatusSiswa"
                        v-bind:class="{ disabled: false }"></v-select>
                    <div>
                        <div class="w-full md:w-72 pt-4 md:pt-2 mx-auto md:mx-0">
                            <div class="relative">
                                ...
                                <input v-model="inputCariSiswa" @keyup="doCariSiswa()" type="search"
                                    class="block p-2 pl-10 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    placeholder="Cari ..." required />
                                <!-- <button
                    type="submit"
              ...
              ></CardCompany>
                        ...
                        <div class="" v-for="(item, index) in dataSiswaCari">
                            <div class="card card-side bg-base-100 shadow-xl h-72" :key="item.id">
                                <figure>
                                    <img src="@/assets/img/avatar/profile-2.svg" alt="Siswa" class="w-32" />
